...
intr_handler (struct intr_frame *frame) 
{
  ...
  handler = intr_handlers[frame->vec_no]; //handlers is an array of function pointers, where each index corresponds to a specific interrupt vector number. The line retrieves the handler function for the current interrupt vector number (frame->vec_no) from the intr_handlers array. If a handler is registered for that interrupt, it will be invoked; otherwise, the code will handle unexpected interrupts or spurious interrupts accordingly.
  if (handler != NULL)
    handler (frame);
